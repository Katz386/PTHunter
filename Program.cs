...
namespace PTHunter
{
    internal class Program
    {
        ...
        [Verb("autopwn", HelpText = "Scan the page for potential vulnerabilities and try to exploit them")]
        public class AutopwnOptions
        {
            [Option("no-banner", HelpText = "Disable welcome banner")]
            public bool NoBanner { get; set; }
        ...
        {
            return CommandLine.Parser.Default.ParseArguments<AttackOptions, DiscoverOptions, AutopwnOptions>(args)
              .MapResult(
                (AttackOptions opts) => Attack.StartAttackAsync(opts).Result,
                (DiscoverOptions opts) => Discover.StartDiscoverAsync(opts).Result,
                (AutopwnOptions opts) => Autopwn.StartAttackAsync(opts).Result,
                errs => 1);
        }
    }
}
